...
const registryDomain = env.REGISTRY_DOMAIN || 'https://mods.airtrfx.com'
...
export const parseVersion = (v) => {
  ...
  ix = v.indexOf('.', lx)
  if (ix === -1) {
    // Handle major.minor format (no patch)
    const min = +v.slice(lx) || 0
    return maj * 1000000 + min * 1000
  }

  const min = +v.slice(lx, ix)
  lx = ix + 1

  const pat = parseInt(v.slice(lx)) || 0
